...
export const deleteCar = async (req, res, next) => {
    try {
        ...
        // if (req.file) {
        //     if (car.image) {
        //         const oldPath = Path2D.join(process.cwd(), 'uploads', car.image);
        //         fs.unlink(oldPath, (err) => { if (err) console.warn('Failed to delete old image:', err); })
        //     }
        //     car.image = req.file.filename
        // }
        if (car.image) {
            ...
            fs.unlink(filePath, (err) => { if (err) console.warn('Failed to delete old image file', err); })
        }
